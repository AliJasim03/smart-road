// src/vehicle.rs - FINAL FIX 2: Corrected geometry for East/West approaches
use crate::intersection::Intersection;
use std::time::Instant;
use std::ops::Sub;

use crate::{LANE_WIDTH, HALF_ROAD_WIDTH, WINDOW_WIDTH, WINDOW_HEIGHT};
...
pub enum VehicleState {
    Approaching, // Moving towards intersection
    Entering,    // Inside intersection, moving to turn point
    Turning,     // At turn point, changing direction
    Exiting,     // Leaving intersection
    Completed,   // Off-screen, to be removed
}
...
pub struct Vehicle {
    pub id: u32,
    pub position: Vec2,
    pub direction: Direction, // Initial direction
    pub destination: Direction, // Final direction after turn
    pub lane: usize,
    pub route: Route,
    ...
    pub turn_point: Vec2,
    pub target_lane_pos: Vec2, // Current target position in lane
    has_reserved_intersection: bool,
    current_movement_dir: Direction, // The direction the vehicle is physically moving right now
...
impl Vehicle {
    ...
    pub const SAFE_DISTANCE: f32 = 60.0;
    ...
    fn calculate_spawn_and_target(direction: Direction, lane: usize) -> (Vec2, Vec2) {
        ...
        match direction {
            Direction::North => (Vec2::new(lane_pos_x, WINDOW_HEIGHT as f32 + spawn_margin), Vec2::new(lane_pos_x, -spawn_margin)),
            Direction::South => (Vec2::new(lane_pos_x, -spawn_margin), Vec2::new(lane_pos_x, WINDOW_HEIGHT as f32 + spawn_margin)),
            Direction::East => (Vec2::new(-spawn_margin, lane_pos_y), Vec2::new(WINDOW_WIDTH as f32 + spawn_margin, lane_pos_y)),
            Direction::West => (Vec2::new(WINDOW_WIDTH as f32 + spawn_margin, lane_pos_y), Vec2::new(-spawn_margin, lane_pos_y)),
        }
    }
    ...
    fn calculate_turn_point(direction: Direction, lane: usize, route: Route) -> Vec2 {
        ...
        match direction {
            // -- The North/South cases are CORRECT as per your last report --
            Direction::North => { // From South
                let x = center_x + lane_center_offset;
                let y = if route == Route::Right { center_y + lane_center_offset } else { center_y - lane_center_offset };
                Vec2::new(x, y)
            },
            Direction::South => { // From North
                let x = center_x - lane_center_offset;
                let y = if route == Route::Right { center_y - lane_center_offset } else { center_y + lane_center_offset };
                Vec2::new(x, y)
            },

            // -- The East/West cases are now fixed --
            Direction::East => { // From West
                let y = center_y + lane_center_offset;
                // Turn RIGHT (to South): needs the nearby vertical lane (x < center).
                // Turn LEFT (to North): needs the faraway vertical lane (x > center).
                let x = if route == Route::Right { center_x - lane_center_offset } else { center_x + lane_center_offset };
                Vec2::new(x, y)
            },
            Direction::West => { // From East
                let y = center_y - lane_center_offset;
                // Turn RIGHT (to North): needs the nearby vertical lane (x > center).
                // Turn LEFT (to South): needs the faraway vertical lane (x < center).
                let x = if route == Route::Right { center_x + lane_center_offset } else { center_x - lane_center_offset };
                Vec2::new(x, y)
            }
        }
    ...
    // Utility functions
    pub fn get_current_movement_direction(&self) -> Direction { self.current_movement_dir }
    pub fn is_in_intersection(&self, intersection: &Intersection) -> bool { intersection.is_point_in_core(self.position.x, self.position.y) }
    pub fn is_approaching_intersection(&self, intersection: &Intersection) -> bool { self.state == VehicleState::Approaching && intersection.is_point_in_approach_zone(self.position.x, self.position.y) }
